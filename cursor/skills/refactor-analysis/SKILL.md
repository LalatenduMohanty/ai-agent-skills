...
6. **Clean code** -- Functions grew longer, gained multiple responsibilities, or added flag arguments that split behavior? Names became less intention-revealing (e.g., `get_` method now has side effects)? Hidden side effects introduced — functions silently modifying state beyond their return value? Bare `except:` clauses, swallowed exceptions, or error codes replacing proper exception handling? Dead code left behind (unused imports, unreachable branches, commented-out blocks)? Comments compensating for unclear code instead of making the code itself clearer?
